...
) => (event: { data: Uint8Array | ArrayBuffer | WebAssembly.Module }) => void;
...
const startRunningService = async (
  ...
  if (useWorker) {
    ...
    let blob = new Blob([`onmessage=${WEB_WORKER_SOURCE_CODE}(postMessage)`], {
      type: "text/javascript",
    });
    worker = new Worker(URL.createObjectURL(blob), { type: "module" });
  } else {
    // Run esbuild on the main thread
    let onmessage = WEB_WORKER_FUNCTION((data: Uint8Array) =>
