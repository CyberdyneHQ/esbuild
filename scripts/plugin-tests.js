...
let pluginTests = {
  ...
  async invalidRegExp({ esbuild }) {
    for (const filter of [/x(?=y)/, /x(?!y)/, /x(?<=y)/, /x(?<!y)/, /(x)\1/]) {
      ...
      try {
        ...
        assert.strictEqual(
          e.message,
          `[name] "onResolve" filter is not a valid Go regular expression: ${JSON.stringify(
            filter.source
          )}`
        );
      }
      ...
      try {
        ...
        assert.strictEqual(
          e.message,
          `[name] "onLoad" filter is not a valid Go regular expression: ${JSON.stringify(
            filter.source
          )}`
        );
      }
  ...
  async badPluginProperty({ esbuild }) {
    try {
      ...
      assert.strictEqual(
        e.errors[0].text,
        'Invalid option on plugin "x": "someRandomProperty"'
      );
      assert.deepStrictEqual(e.warnings, []);
    }
  },
  ...
  async badPluginOnResolveProperty({ esbuild }) {
    try {
      ...
      assert.strictEqual(
        e.errors[0].text,
        'Invalid option in onResolve() call for plugin "x": "whatIsThis"'
      );
      assert.deepStrictEqual(e.warnings, []);
    }
    ...
    try {
      ...
      assert(
        e.message.endsWith(
          'ERROR: [plugin: x] Expected onResolve() callback in plugin "x" to return an object'
        ),
        e.message
      );
    }
    ...
    try {
      ...
      assert(
        e.message.endsWith(
          'ERROR: [plugin: x] Invalid option from onResolve() callback in plugin "x": "thisIsWrong"'
        ),
        e.message
      );
    }
  ...
  async badPluginOnLoadProperty({ esbuild }) {
    try {
      ...
      assert.strictEqual(
        e.errors[0].text,
        'Invalid option in onLoad() call for plugin "x": "whatIsThis"'
      );
      assert.deepStrictEqual(e.warnings, []);
    }
    ...
    try {
      ...
      assert(
        e.message.endsWith(
          `ERROR: [plugin: x] Expected onLoad() callback in plugin "x" to return an object`
        ),
        e.message
      );
    }
    ...
    try {
      ...
      assert(
        e.message.endsWith(
          'ERROR: [plugin: x] Invalid option from onLoad() callback in plugin "x": "thisIsWrong"'
        ),
        e.message
      );
    }
  ...
  async resolversCalledInSequence({ esbuild, testDir }) {
    ...
    await esbuild.build({
      ...
      plugins: [
        ...
        },
        {
          name: "plugin2",
          setup(build) {
            build.onResolve({ filter: /^ignore me$/ }, () => {
              trace.push("not called");
            });
          },
        },
        {
    ...
    const result = require(output);
    assert.strictEqual(result.default, 123);
    assert.deepStrictEqual(trace, ["called first", "called second"]);
  },
      ...
      plugins: [
        {
          ...
          setup(build) {
            ...
            build.onLoad(
              { filter: /^http:\/\//, namespace: "http" },
              (args) => {
                switch (args.path) {
                  case "http://example.com/assets/js/example.js":
                    return {
                      contents: `import y from './data/base.js'; export default y`,
                    };
  ...
  async rewriteExternalWithFileNamespace({ esbuild, testDir }) {
    ...
    await fs.promises.rename(
      path.join(outdir, "in.js"),
      path.join(outdir2, "in.js")
    );
  ...
  async resolveWithSideEffectsFalse({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      path.join(testDir, "re-export-unused.js"),
      `
    ...
    await writeFileAsync(
      path.join(testDir, "re-export-used.js"),
      `
    ...
    await writeFileAsync(
      path.join(testDir, "import-unused.js"),
      `
          ...
          setup(build) {
            build.onResolve({ filter: /^plugin:/ }, (args) => {
              return {
                path: args.path,
                namespace: "ns",
                sideEffects: args.path.endsWith("-true")
                  ? true
                  : args.path.endsWith("-false")
                  ? false
                  : undefined,
              };
            });
            build.onLoad({ filter: /^plugin:/ }, (args) => {
              return {
                contents: `export default use(${JSON.stringify(args.path)})`,
              };
            });
          },
        },
      ...
      plugins: [
        {
          ...
          setup(build) {
            ...
            build.onLoad({ filter: /.*/, namespace: "for-testing" }, () => ({
              contents,
              resolveDir,
            }));
          },
      ...
      plugins: [
        {
          ...
          setup(build) {
            build.onResolve({ filter: /\.wasm$/ }, (args) => ({
              ...
              namespace:
                args.namespace === "wasm-stub" ? "wasm-binary" : "wasm-stub",
            }));
            build.onLoad(
  ...
  async virtualEntryPoints({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      result.outputFiles[2].path,
      path.join(testDir, "a_b.js")
    );
    assert.strictEqual(
      result.outputFiles[3].path,
      path.join(testDir, "a/b/c.d.js")
    );
    assert.strictEqual(
      ...
      plugins: [
        {
          ...
          setup(build) {
            build.onResolve({ filter: /^plugin$/ }, (args) => {
              ...
              assert.strictEqual(
                args.importer,
                path.join(testDir, "stdin-sourcefile")
              );
  ...
  async externalRequire({ esbuild, testDir }) {
    const externalPlugin = (external) => ({
      ...
      setup(build) {
        ...
        build.onLoad({ filter: /.*/, namespace: "external" }, (args) => ({
          contents: `import * as all from ${JSON.stringify(
            args.path
          )}; module.exports = all`,
        }));
      },
    ...
    await esbuild.build({
      ...
      outfile,
      format: "esm",
      plugins: [externalPlugin(["fs", "url", "path"])],
    });
      ...
      plugins: [
        {
          ...
          setup(build) {
            build.onResolve({ filter: /.*/ }, (args) => {
              if (args.path === "entry")
                return { path: "entry", namespace: "xyz" };
              return {
                path: "nested",
                namespace: "xyz",
                pluginData: theObject,
        ...
        plugins: [
          {
            ...
            setup(build) {
              ...
              build.onLoad({ filter: /.*/, namespace: "ns" }, () => {
                return {
                  contents: `div { background: url('test') }`,
                  loader: "css",
                };
              });
            },
  ...
  async onResolvePreserveOriginalEntryPointNameIssue945({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      build.outputFiles[0].path,
      path.join(testDir, "first.js")
    );
  },
  ...
  async dynamicImportDuplicateChunkIssue1099({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      ...
      `import x from 'manifest'; console.log(x.name(), x.hi())`
    );
    await writeFileAsync(
      path.join(testDir, "name.js"),
      `import x from 'manifest'; console.log(x.index(), x.hi())`
    );
    await writeFileAsync(
      path.join(testDir, "hi", "name.js"),
      `import x from 'manifest'; console.log(x.index(), x.name())`
    );
    await esbuild.build({
...
error: Invalid path suffix "%what" returned from plugin (must start with "?" or "#")`
  ...
  async onResolveWithInternalOnLoadAndQuerySuffix({ testDir, esbuild }) {
    ...
    const expected = JSON.stringify(
      {
        onResolve: [{ path: "foo%a" }, { path: "foo%b" }],
        onLoad: [
    ...
    if (observed !== expected)
      throw new Error(`Observed ${observed}, expected ${expected}`);
  },
  ...
  async onLoadWithInternalOnResolveAndQuerySuffix({ testDir, esbuild }) {
    ...
    const expected = JSON.stringify(
      {
        onResolve: [{ path: "./entry?a" }, { path: "./entry?b" }],
        onLoad: [
          { path: path.join(testDir, "entry.js"), suffix: "?a" },
          { path: path.join(testDir, "entry.js"), suffix: "?b" },
        ],
            ...
            build.onResolve({ filter: /.*/ }, (args) => {
              if (args.importer === "")
                ...
                return {
                  path: args.path,
                  external: true,
                  sideEffects: args.path !== "noSideEffects",
                };
            });
            build.onLoad({ filter: /.*/, namespace: "entry" }, () => {
              return {
  ...
  async callResolveTooLateError({ esbuild }) {
    ...
    try {
      ...
      assert(
        e.message.includes('Cannot call "resolve" on an inactive build'),
        e.message
      );
    }
  },
      ...
      plugins: [
        {
          ...
          setup(build) {
            ...
            build.onResolve({ filter: /^foo$/ }, async (args) => {
              ...
              assert.strictEqual(
                args.resolveDir,
                path.join(process.cwd(), "foo-resolveDir")
              );
...
let syncTests = {
  async pluginWithWatchMode({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      input,
      `import {x} from "./example.js"; exports.x = x`
    );
          ...
          setup(build) {
            ...
            build.onLoad(
              ...
              async () => {
                ...
                return {
                  contents: `export let x = ${entries.length}`,
                  watchDirs: [otherDir],
                };
              }
            );
    ...
    try {
      ...
      // First rebuild: edit
      {
        const [error2, result2] = await rebuildUntil(
          () =>
            setTimeout(
              () => writeFileAtomic(path.join(otherDir, "file.txt"), `...`),
              250
            ),
          () => fs.readFileSync(outfile, "utf8") !== code
        );
  ...
  async onStartCallback({ esbuild, testDir }) {
    ...
    valueToReturn = {
      errors: [{ text: "return test", location: { file: "foo.js", line: 2 } }],
    };
    try {
      await result.rebuild();
      throw new Error("Expected an error to be thrown");
    ...
    assert.strictEqual(onStartTimes, 3);
    valueToReturn = new Promise((resolve) =>
      setTimeout(() => {
        onStartTimes++;
        resolve();
      }, 500)
...
async function main() {
  ...
  let timeout = setTimeout(() => {
    console.error(
      `❌ plugin tests timed out after ${minutes} minutes, exiting...`
    );
    process.exit(1);
  }, minutes * 60 * 1000);
