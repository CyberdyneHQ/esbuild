...
const buildNeutralLib = (esbuildPath) => {
  ...
  const types_ts = fs.readFileSync(
    path.join(repoDir, "lib", "shared", "types.ts"),
    "utf8"
  );
...
function generateWorkerCode({ esbuildPath, wasm_exec_js, minify, target }) {
  ...
  `;
  const wasmExecAndWorker = childProcess
    .execFileSync(
      esbuildPath,
      [
        "--loader=ts",
  ...
  const firstNonComment = commentLines.findIndex(
...
exports.buildWasmLib = async (esbuildPath) => {
  ...
  const goBuildPromise = new Promise((resolve, reject) =>
    childProcess.execFile(
      "go",
      [
        "build",
        "-o",
        path.join(npmWasmDir, "esbuild.wasm"),
  ...
  const GOROOT = childProcess
    ...
    .toString()
    .trim();
  let wasm_exec_js = fs.readFileSync(
    path.join(GOROOT, "misc", "wasm", "wasm_exec.js"),
    "utf8"
  );
  let wasm_exec_node_js = fs.readFileSync(
    path.join(GOROOT, "misc", "wasm", "wasm_exec_node.js"),
    "utf8"
  );
  fs.writeFileSync(path.join(npmWasmDir, "wasm_exec.js"), wasm_exec_js);
  fs.writeFileSync(
    path.join(npmWasmDir, "wasm_exec_node.js"),
    wasm_exec_node_js
  );
  ...
  for (const minify of [false, true]) {
    ...
    const wasmWorkerCodeUMD = generateWorkerCode({
      esbuildPath,
      wasm_exec_js,
      minify,
      target: umdBrowserTarget,
    });
    const wasmWorkerCodeESM = generateWorkerCode({
      esbuildPath,
      wasm_exec_js,
      minify,
    ...
    const browserCJS = childProcess
      .execFileSync(
        ...
        [
          ...
          "--format=cjs",
          "--define:ESBUILD_VERSION=" + JSON.stringify(version),
          "--define:WEB_WORKER_SOURCE_CODE=" +
            JSON.stringify(wasmWorkerCodeUMD),
          "--banner:js=" + umdPrefix,
          "--footer:js=" + umdSuffix,
          "--log-level=warning",
        ].concat(minifyFlags),
      ...
      .toString()
      .replace("WEB_WORKER_FUNCTION", wasmWorkerCodeUMD);
    fs.writeFileSync(
      path.join(libDir, minify ? "browser.min.js" : "browser.js"),
      browserCJS
    );

    // Generate "npm/esbuild-wasm/esm/browser.min.js"
    const browserESM = childProcess
      .execFileSync(
        esbuildPath,
        [
          path.join(repoDir, "lib", "npm", "browser.ts"),
          ...
          "--define:ESBUILD_VERSION=" + JSON.stringify(version),
          "--define:WEB_WORKER_SOURCE_CODE=" +
            JSON.stringify(wasmWorkerCodeESM),
          "--log-level=warning",
        ].concat(minifyFlags),
        { cwd: repoDir }
      )
      .toString()
      .replace("WEB_WORKER_FUNCTION", wasmWorkerCodeESM);
    fs.writeFileSync(
      path.join(esmDir, minify ? "browser.min.js" : "browser.js"),
      browserESM
    );
  }
  ...
  for (const entry of fs.readdirSync(exit0Dir)) {
    if (entry.endsWith(".node")) {
      ...
      const compressed = zlib.deflateRawSync(fs.readFileSync(absPath), {
        level: 9,
      });
...
`;
  ...
  for (const dir of [path.join(repoDir, "npm", "esbuild-android-64")]) {
    ...
    fs.copyFileSync(
      path.join(npmWasmDir, "bin", "esbuild"),
      path.join(dir, "bin", "esbuild")
    );
    fs.copyFileSync(
      path.join(npmWasmDir, "esbuild.wasm"),
      path.join(dir, "esbuild.wasm")
    );
  }
};
...
const buildDenoLib = (esbuildPath) => {
  ...
  const wasmWorkerCode = generateWorkerCode({
    esbuildPath,
    wasm_exec_js,
    minify: true,
    target: "esnext",
  });
  const modWASM = childProcess
    .execFileSync(
      esbuildPath,
      [
    ...
    .toString()
    .replace("WEB_WORKER_FUNCTION", wasmWorkerCode);
  fs.writeFileSync(path.join(denoDir, "wasm.js"), modWASM);

  // Generate "deno/esbuild/mod.d.ts"
  const types_ts =
    fs.readFileSync(path.join(repoDir, "lib", "shared", "types.ts"), "utf8") +
  ...
  fs.copyFileSync(
    path.join(repoDir, "npm", "esbuild-wasm", "esbuild.wasm"),
    path.join(repoDir, "deno", "esbuild.wasm")
  );
};
...
exports.writeFileAtomic = (where, contents) => {
  ...
  const file = path.join(
    path.dirname(where),
    ".esbuild-atomic-file-" + Math.random().toString(36).slice(2)
  );
...
exports.buildBinary = () => {
  childProcess.execFileSync(
    "go",
    ["build", "-ldflags=-s -w", "-trimpath", "./cmd/esbuild"],
...
const updateVersionPackageJSON = (pathToPackageJSON) => {
  const version = fs
    .readFileSync(path.join(path.dirname(__dirname), "version.txt"), "utf8")
    .trim();
...
exports.installForTests = () => {
  ...
  childProcess.execSync(`npm pack --silent "${npmDir}"`, {
    cwd: installDir,
    stdio: "inherit",
  });
  childProcess.execSync(
    `npm install --silent --no-audit --progress=false esbuild-${version}.tgz`,
    { cwd: installDir, env, stdio: "inherit" }
  );

  // Evaluate the code
  const ESBUILD_PACKAGE_PATH = path.join(installDir, "node_modules", "esbuild");
  const mod = require(ESBUILD_PACKAGE_PATH);
  Object.defineProperty(mod, "ESBUILD_PACKAGE_PATH", {
    value: ESBUILD_PACKAGE_PATH,
  });
  return mod;
};

const updateVersionGo = () => {
  const version_txt = fs
    .readFileSync(path.join(repoDir, "version.txt"), "utf8")
    .trim();
...
if (require.main === module) {
  ...
  else if (process.argv.indexOf("--version") >= 0)
    updateVersionPackageJSON(process.argv[2]);
  else if (process.argv.indexOf("--neutral") >= 0)
    buildNeutralLib(process.argv[2]);
