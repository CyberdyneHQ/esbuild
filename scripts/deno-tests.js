...
const wasmModule = await WebAssembly.compile(
  await Deno.readFile(path.join(__dirname, "..", "deno", "esbuild.wasm"))
);
...
test("basicBuild", ["native"], async ({ esbuild, testDir }) => {
  ...
  await Deno.writeTextFile(
    input,
    'import dep from "./dep.ts"; export default dep === 123'
  );
...
test(
  ...
  async ({ esbuild }) => {
      ...
      plugins: [
        {
          ...
          setup(build) {
            ...
            build.onLoad({ filter: /^<entry>$/ }, () => ({
              contents: `import dep from "<dep>"; export default dep === 123`,
            }));
            build.onLoad({ filter: /^<dep>$/ }, () => ({
    ...
    const result = await import(
      "data:application/javascript;base64," + btoa(build.outputFiles[0].text)
    );
    asserts.assertStrictEquals(result.default, true);
  }
);
