...
let buildTests = {
  async errorIfEntryPointsNotArray({ esbuild }) {
    try {
      ...
      if (
        !e.errors ||
        !e.errors[0] ||
        e.errors[0].text !== '"entryPoints" must be an array or an object'
      ) {
        throw e;
      }
    }
  ...
  async errorIfGlob({ esbuild }) {
    try {
      ...
      if (
        !e.errors ||
        !e.errors[0] ||
        e.errors[0].text !== 'Could not resolve "./src/*.js"' ||
        e.errors[0].notes[0].text !==
  ...
  async sourceMapWithDisabledModule({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      json.sources[0],
      path.relative(testDir, disabled).split(path.sep).join("/")
    );
    assert.strictEqual(json.sources[1], path.basename(input));
    assert.strictEqual(json.sourcesContent[0], "");
    assert.strictEqual(json.sourcesContent[1], content);
  ...
  async requireAbsolutePath({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      input,
      `import value from ${JSON.stringify(dependency)}; export default value`
    );
  ...
  async fileLoader({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      input,
      `export {default as value} from ${JSON.stringify(data)}`
    );
  ...
  async fileLoaderEntryHash({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      new Set(
        result1.outputFiles.concat(result2.outputFiles).map((x) => x.path)
      ).size,
      4
    );
  },
  ...
  async splittingPublicPath({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      input1,
      `export {default as input1} from ${JSON.stringify(shared)}`
    );
    await writeFileAsync(
      input2,
      `export {default as input2} from ${JSON.stringify(shared)}`
    );
    await writeFileAsync(
      shared,
      `export default function foo() { return 123 }`
    );
    const value = await esbuild.build({
    ...
    assert.deepStrictEqual(
      value.outputFiles[0].path,
      path.join(outdir, "a", "in1.js")
    );
    assert.deepStrictEqual(
      value.outputFiles[1].path,
      path.join(outdir, "b", "in2.js")
    );
    assert.deepStrictEqual(
      value.outputFiles[2].path,
      path.join(outdir, "chunk-22JQAFLY.js")
    );
    assert.deepStrictEqual(
...
`
    ...
    assert.deepStrictEqual(
      value.outputFiles[2].text,
      `// scripts/.js-api-tests/splittingPublicPath/c/shared.js
function foo() {
  return 123;
}
...
`
  ...
  async fileLoaderPublicPath({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      result.value,
      "https://www.example.com/assets/data-BYATPJRB.bin"
    );
    assert.strictEqual(result.__esModule, true);
  },
  ...
  async fileLoaderCSS({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      input,
      `body { background: url(${JSON.stringify(data)}) }`
    );
    await writeFileAsync(data, `stuff`);
    const value = await esbuild.build({
      entryPoints: [input],
    ...
    assert.strictEqual(value.outputFiles, void 0);
    assert.strictEqual(
      await readFileAsync(output, "utf8"),
      `/* scripts/.js-api-tests/fileLoaderCSS/in.css */
body {
...
`
  ...
  async fileLoaderWithAssetPathAndPublicPath({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      result.value,
      "https://www.example.com/assets/name=data/hash=BYATPJRB.bin"
    );
    assert.strictEqual(result.__esModule, true);
    const stuff = fs.readFileSync(
      path.join(outdir, "assets", "name=data", "hash=BYATPJRB.bin"),
      "utf8"
    );
    assert.strictEqual(stuff, "stuff");
  },
  ...
  async metafile({ esbuild, testDir }) {
    ...
    const makePath = (absPath) =>
      path.relative(cwd, absPath).split(path.sep).join("/");
    ...
    assert.strictEqual(
      typeof json.outputs[makePath(outputCSS)].bytes,
      "number"
    );
    assert.strictEqual(
      typeof json.outputs[makePath(outputJS) + ".map"].bytes,
      "number"
    );
    assert.strictEqual(
      typeof json.outputs[makePath(outputCSS) + ".map"].bytes,
      "number"
    );
    assert.strictEqual(
      json.outputs[makePath(outputJS)].entryPoint,
      makePath(entry)
    );
    assert.strictEqual(json.outputs[makePath(outputCSS)].entryPoint, undefined); // This is deliberately undefined
    assert.deepStrictEqual(
      json.outputs[makePath(outputJS) + ".map"].imports,
      []
    );
    assert.deepStrictEqual(
      json.outputs[makePath(outputJS) + ".map"].exports,
      []
    );
    assert.deepStrictEqual(
      json.outputs[makePath(outputJS) + ".map"].inputs,
      {}
    );
    assert.deepStrictEqual(
      json.outputs[makePath(outputCSS) + ".map"].imports,
      []
    );
    assert.deepStrictEqual(
      json.outputs[makePath(outputCSS) + ".map"].exports,
      []
    );
    assert.deepStrictEqual(
      json.outputs[makePath(outputCSS) + ".map"].inputs,
      {}
    );
    ...
    assert.strictEqual(
      typeof outputInputs[makePath(entry)].bytesInOutput,
      "number"
    );
    assert.strictEqual(
      typeof outputInputs[makePath(imported)].bytesInOutput,
      "number"
    );
    assert.strictEqual(
      typeof outputInputs[makePath(text)].bytesInOutput,
      "number"
    );
    assert.strictEqual(
      typeof outputInputs[makePath(css)].bytesInOutput,
      "number"
    );
  },
  ...
  async metafileSplitting({ esbuild, testDir }) {
    ...
    const cwd = process.cwd();
    const makeOutPath = (basename) =>
      path.relative(cwd, path.join(outdir, basename)).split(path.sep).join("/");
    const makeInPath = (pathname) =>
      path.relative(cwd, pathname).split(path.sep).join("/");
    ...
    assert.deepStrictEqual(json.inputs[inEntry1], {
      bytes: 94,
      imports: [{ path: inImported, kind: "import-statement" }],
    });
    ...
    assert.deepStrictEqual(json.outputs[outEntry1].imports, [
      { path: makeOutPath(chunk), kind: "import-statement" },
    ]);
    assert.deepStrictEqual(json.outputs[outEntry2].imports, [
      { path: makeOutPath(chunk), kind: "import-statement" },
    ]);
    ...
    assert.deepStrictEqual(json.outputs[outEntry1].inputs, {
      [inEntry1]: { bytesInOutput: 40 },
    });
    assert.deepStrictEqual(json.outputs[outEntry2].inputs, {
      [inEntry2]: { bytesInOutput: 48 },
    });
    assert.deepStrictEqual(json.outputs[outChunk].inputs, {
      [inImported]: { bytesInOutput: 87 },
    });
  },
  ...
  async metafileSplittingDoubleDynamicImport({ esbuild, testDir }) {
    ...
    const outdir = path.join(testDir, "out");
    const makeImportPath = (importing, imported) =>
      JSON.stringify(
        "./" +
          path
            .relative(path.dirname(importing), imported)
            .split(path.sep)
            .join("/")
      );
    ...
    assert.deepStrictEqual(json.inputs[inImport1], {
      bytes: 35,
      imports: [{ path: inShared, kind: "import-statement" }],
    });
    ...
    assert.deepStrictEqual(json.outputs[outImport1].imports, [
      { path: outChunk, kind: "import-statement" },
    ]);
    assert.deepStrictEqual(json.outputs[outImport2].imports, [
      { path: outChunk, kind: "import-statement" },
    ]);
    assert.deepStrictEqual(json.outputs[outChunk].imports, []);
    ...
    assert.deepStrictEqual(json.outputs[outEntry].inputs, {
      [inEntry]: { bytesInOutput: 74 },
    });
    assert.deepStrictEqual(json.outputs[outImport1].inputs, {
      [inImport1]: { bytesInOutput: 0 },
    });
    assert.deepStrictEqual(json.outputs[outImport2].inputs, {
      [inImport2]: { bytesInOutput: 0 },
    });
    assert.deepStrictEqual(json.outputs[outChunk].inputs, {
      [inShared]: { bytesInOutput: 28 },
    });
  },
  ...
  async metafileCJSInFormatCJS({ esbuild, testDir }) {
    ...
    assert.deepStrictEqual(json.outputs[makePath(outfile)].exports, []);
    assert.deepStrictEqual(
      json.outputs[makePath(outfile)].entryPoint,
      makePath(entry)
    );
  },
  ...
  async metafileCSS({ esbuild, testDir }) {
    ...
    assert.deepStrictEqual(json, {
      inputs: {
        [makePath(entry)]: {
          bytes: 98,
          imports: [{ path: makePath(imported), kind: "import-rule" }],
        },
        [makePath(image)]: { bytes: 8, imports: [] },
        [makePath(imported)]: {
          bytes: 48,
          imports: [{ path: makePath(image), kind: "url-token" }],
        },
      },
      outputs: {
        [makePath(output)]: {
  ...
  async metafileLoaderFileMultipleEntry({ esbuild, testDir }) {
    ...
    assert.deepStrictEqual(json.outputs[fileKey].inputs, {
      [makePath(file)]: { bytesInOutput: 14 },
    });
  },
  ...
  async writeFalse({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      meta.outputs[makePath(output + ".map")].bytes,
      value.outputFiles[0].contents.length
    );
  },
  ...
  async allowOverwrite({ esbuild, testDir }) {
    ...
    try {
      ...
      if (
        ...
        !e.errors ||
        !e.errors.length ||
        !e.errors[0].text.includes("Refusing to overwrite input file")
      )
        throw e;
    }
...
`
    ...
    assert.strictEqual(
      Buffer.from(value.outputFiles[1].contents).toString(),
      `import {
  common_default
} from "./${chunk}";
...
`
    ...
    assert.strictEqual(
      Buffer.from(value.outputFiles[2].contents).toString(),
      `// scripts/.js-api-tests/splittingRelativeSameDir/common.js
...
`
    ...
    assert.strictEqual(
      value.outputFiles[0].path,
      path.join(outdir, path.basename(inputA))
    );
    assert.strictEqual(
      value.outputFiles[1].path,
      path.join(outdir, path.basename(inputB))
    );
...
`
    ...
    assert.strictEqual(
      Buffer.from(value.outputFiles[2].contents).toString(),
      `// scripts/.js-api-tests/splittingRelativeNestedDir/common.js
var common_default = "common";
...
`
    ...
    assert.strictEqual(
      value.outputFiles[0].path,
      path.join(outdir, path.relative(testDir, inputA))
    );
    assert.strictEqual(
      value.outputFiles[1].path,
      path.join(outdir, path.relative(testDir, inputB))
    );
    assert.strictEqual(value.outputFiles[2].path, path.join(outdir, chunk));
  },
...
`
    ...
    assert.strictEqual(
      value.outputFiles[2].text,
      `// scripts/.js-api-tests/splittingWithChunkPath/common.js
var common_default = "common";
...
`
    ...
    assert.strictEqual(
      value.outputFiles[2].text,
      `// scripts/.js-api-tests/splittingWithEntryHashes/common.js
...
`
  ...
  async splittingWithChunkPathAndCrossChunkImportsIssue899({
    esbuild,
    testDir,
  }) {
    ...
    // This needs to use relative paths to avoid breaking on Windows.
    // Importing by absolute path doesn't work on Windows in node.
    const result1 = await import(
      "./" + path.relative(__dirname, path.join(outdir, "entry1.mjs"))
    );
    const result2 = await import(
      "./" + path.relative(__dirname, path.join(outdir, "entry2.mjs"))
    );
    const result3 = await import(
      "./" + path.relative(__dirname, path.join(outdir, "entry3.mjs"))
    );
...
`
  ...
  async forceTsConfig({ esbuild, testDir }) {
    ...
    await writeFileAsync(
      tsconfigIgnore,
      '{"compilerOptions": {"baseUrl": "./a", "paths": {"test": ["./ignore.js"]}}}'
    );
    await writeFileAsync(
      tsconfigForced,
      '{"compilerOptions": {"baseUrl": "./b", "paths": {"test": ["./test-impl.js"]}}}'
    );
    await esbuild.build({
      entryPoints: [input],
      bundle: true,
    ...
    assert.strictEqual(
      result,
      `// scripts/.js-api-tests/forceTsConfig/a/b/test-impl.js
console.log("success");
`
    );
  },
...
`
  ...
  async buildRelativeIssue693({ esbuild }) {
    ...
    assert.strictEqual(
      ...
      path.join(process.cwd(), "esbuild.js")
    );
  ...
  async rebuildIndependent({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      await readFileAsync(outputA, "utf8"),
      `console.log("a");\n`
    );
    assert.strictEqual(
      await readFileAsync(outputB, "utf8"),
      `console.log("b");\n`
    );
  ...
  async customEntryPointOutputPathsRel({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      outputFiles[0].path,
      path.join(testDir, "entry", "out", "CXHWNMAN-1.cjs.js")
    );
    assert.strictEqual(
      outputFiles[1].path,
      path.join(testDir, "entry", "out", "EYSNILNO-2.mjs.js")
    );
  },
  ...
  async customEntryPointOutputPathsAbs({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      outputFiles[0].path,
      path.join(testDir, "entry", "out", "TIORPBNU-1.js")
    );
    assert.strictEqual(
      outputFiles[1].path,
      path.join(testDir, "entry", "out", "3KY7NOSR-2.js")
    );
  },
  ...
  async nodeColonPrefixImport({ esbuild }) {
    const tryTargetESM = async (target) => {
      const result = await esbuild.build({
        stdin: {
          contents: `import fs from 'node:fs'; import('node:fs'); fs()`,
        },
        bundle: true,
        platform: "node",
        target,
    ...
    assert.strictEqual(
      await tryTargetESM("node14.13.1"),
      `// <stdin>\nimport fs from "node:fs";\nimport("node:fs");\nfs();\n`
    );
    assert.strictEqual(
      await tryTargetESM("node14.13.0"),
      `// <stdin>\nimport fs from "fs";\nimport("fs");\nfs();\n`
    );
    assert.strictEqual(
      await tryTargetESM("node13"),
      `// <stdin>\nimport fs from "fs";\nPromise.resolve().then(() => __toESM(__require("fs")));\nfs();\n`
    );
    assert.strictEqual(
      await tryTargetESM("node12.99"),
      `// <stdin>\nimport fs from "node:fs";\nimport("node:fs");\nfs();\n`
    );
    assert.strictEqual(
      await tryTargetESM("node12.20"),
      `// <stdin>\nimport fs from "node:fs";\nimport("node:fs");\nfs();\n`
    );
    assert.strictEqual(
      await tryTargetESM("node12.19"),
      `// <stdin>\nimport fs from "fs";\nPromise.resolve().then(() => __toESM(__require("fs")));\nfs();\n`
    );
  },
  ...
  async nodeColonPrefixRequire({ esbuild }) {
    ...
    assert.strictEqual(
      await tryTargetESM("node16"),
      `// <stdin>\nrequire("node:fs");\nrequire.resolve("node:fs");\n`
    );
    assert.strictEqual(
      await tryTargetESM("node15.99"),
      `// <stdin>\nrequire("fs");\nrequire.resolve("fs");\n`
    );
    assert.strictEqual(
      await tryTargetESM("node15"),
      `// <stdin>\nrequire("fs");\nrequire.resolve("fs");\n`
    );
    assert.strictEqual(
      await tryTargetESM("node14.99"),
      `// <stdin>\nrequire("node:fs");\nrequire.resolve("node:fs");\n`
    );
    assert.strictEqual(
      await tryTargetESM("node14.18"),
      `// <stdin>\nrequire("node:fs");\nrequire.resolve("node:fs");\n`
    );
    assert.strictEqual(
      await tryTargetESM("node14.17"),
      `// <stdin>\nrequire("fs");\nrequire.resolve("fs");\n`
    );
  },
...
function fetch(host, port, path, headers) {
  return new Promise((resolve, reject) => {
    http
      .get({ host, port, path, headers }, (res) => {
        const chunks = [];
        ...
        res.on("end", () => {
          ...
          if (res.statusCode < 200 || res.statusCode > 299)
            reject(
              new Error(`${res.statusCode} when fetching ${path}: ${content}`)
            );
          else {
  ...
  async watchEditSession({ esbuild, testDir }) {
    ...
    const rebuildUntil = (mutator, condition) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        onRebuild = (args) => {
          try {
            if (condition(...args)) clearTimeout(timeout), resolve(args);
          } catch (e) {
...
let serveTests = {
  ...
  async serveWithFallbackDirAndSiblingOutputDir({ esbuild, testDir }) {
    try {
      ...
      assert.strictEqual(
        e + "",
        `Error: Output directory "out" must be contained in serve directory "www"`
      );
    }
  },
  ...
  async serveWithFallbackDirAndParentOutputDir({ esbuild, testDir }) {
    try {
      ...
      assert.strictEqual(
        e + "",
        `Error: Output directory "." must be contained in serve directory "www"`
      );
    }
  },
  ...
  async serveWithFallbackDirNoEntryPoints({ esbuild, testDir }) {
    ...
    assert.strictEqual(
      buffer.toString(),
      `<!doctype html><meta charset="utf8"><title>Directory: /</title><h1>Directory: /</h1><ul></ul>`
    );
    req = await promise;
    assert.strictEqual(req.method, "GET");
    assert.strictEqual(req.path, "/");
  ...
  async serveRange({ esbuild, testDir }) {
    ...
    for (let i = 0, n = 16; i < n; i++) {
      const length = Math.round(
        minLength + ((maxLength - minLength) * i) / (n - 1)
      );
...
async function futureSyntax(esbuild, js, targetBelow, targetAbove) {
  failure: {
    try {
      await esbuild.transform(js, { target: targetBelow });
    } catch {
      break failure;
    }
  ...
  try {
    await esbuild.transform(js, { target: targetAbove });
  } catch (e) {
    throw new Error(`Expected success for ${targetAbove}: ${js}\n${e}`);
  }
}
...
let transformTests = {
  ...
  async version({ esbuild }) {
    const version = fs
      .readFileSync(path.join(repoDir, "version.txt"), "utf8")
      .trim();
  ...
  async throwOnBadOptions({ esbuild }) {
    ...
    try {
      ...
      if (
        !e.errors ||
        !e.errors[0] ||
        e.errors[0].text !== '"jsxFactory" must be a string'
      ) {
        throw e;
      }
    }
  ...
  async mangleCacheTransform({ esbuild }) {
    var { code, mangleCache } = await esbuild.transform(
      `x = { x_: 0, y_: 1, z_: 2 }`,
      {
        mangleProps: /_/,
        mangleCache: { x_: "FIXED", z_: false },
  ...
  async tsconfigRawPreserveValueImportsTrueImportsNotUsedAsValuesRemove({
    esbuild,
  }) {
    const { code } = await esbuild.transform(`import {T} from 'path'`, {
      tsconfigRaw: {
  ...
  async tsconfigRawJSX({ esbuild }) {
    ...
    assert.strictEqual(
      code1,
      `/* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", null));\n`
    );
    ...
    assert.strictEqual(
      code2,
      `/* @__PURE__ */ factory(fragment, null, /* @__PURE__ */ factory("div", null));\n`
    );
  },
  ...
  async cssCharsetDefault({ esbuild }) {
    const { code } = await esbuild.transform(`.π:after { content: 'π' }`, {
      loader: "css",
    });
  ...
  async cssMinify({ esbuild }) {
    const { code } = await esbuild.transform(`div { color: #abcd }`, {
      loader: "css",
      minify: true,
    });
  ...
  async es6_import_to_iife({ esbuild }) {
    const { code } = await esbuild.transform(
      `import {exists} from "fs"; if (!exists) throw 'fail'`,
      { format: "iife" }
    );
  ...
  async es6_import_star_to_iife({ esbuild }) {
    const { code } = await esbuild.transform(
      `import * as fs from "fs"; if (!fs.exists) throw 'fail'`,
      { format: "iife" }
    );
    new Function("require", code)(require);
  },
  ...
  async es6_export_to_iife({ esbuild }) {
    const { code } = await esbuild.transform(`export {exists} from "fs"`, {
      format: "iife",
      globalName: "out",
    });
  ...
  async es6_export_star_to_iife({ esbuild }) {
    const { code } = await esbuild.transform(`export * from "fs"`, {
      format: "iife",
      globalName: "out",
    });
    const out = new Function("require", code + ";return out")(require);
  ...
  async es6_export_star_as_to_iife({ esbuild }) {
    const { code } = await esbuild.transform(`export * as fs from "fs"`, {
      format: "iife",
      globalName: "out",
    });
    const out = new Function("require", code + ";return out")(require);
  ...
  async iifeGlobalName({ esbuild }) {
    const { code } = await esbuild.transform(`export default 123`, {
      format: "iife",
      globalName: "testName",
    });
    const globals = {};
...
π["π 𐀀"]["𐀀"]["𐀀 π"] = `
  ...
  async jsx({ esbuild }) {
    const { code } = await esbuild.transform(`console.log(<div/>)`, {
      loader: "jsx",
    });
    assert.strictEqual(
      code,
  ...
  async ts({ esbuild }) {
    const { code } = await esbuild.transform(`enum Foo { FOO }`, {
      loader: "ts",
    });
    assert.strictEqual(
      code,
  ...
  async tsx({ esbuild }) {
    const { code } = await esbuild.transform(`console.log(<Foo<T>/>)`, {
      loader: "tsx",
    });
    assert.strictEqual(
      code,
  ...
  async minify({ esbuild }) {
    const { code } = await esbuild.transform(`console.log("a" + "b" + c)`, {
      minify: true,
    });
  ...
  async keepConsole({ esbuild }) {
    const { code } = await esbuild.transform(`console.log('foo')`, {
      drop: [],
    });
  ...
  async dropDebugger({ esbuild }) {
    const { code } = await esbuild.transform(`if (x) debugger`, {
      drop: ["debugger"],
    });
    assert.strictEqual(code, `if (x)\n  ;\n`);
  },
  ...
  async define({ esbuild }) {
    ...
    const { code: code1 } = await esbuild.transform(
  ...
  async defineArray({ esbuild }) {
    ...
    assert.strictEqual(
      code,
      `var define_process_env_NODE_ENV_default = [1, 2, 3];\nconsole.log(define_process_env_NODE_ENV_default);\n`
    );
  },

  async defineThis({ esbuild }) {
    const { code } = await esbuild.transform(`console.log(a, b); export {}`, {
  ...
  async json({ esbuild }) {
    const { code } = await esbuild.transform(`{ "x": "y" }`, {
      loader: "json",
    });
  ...
  async jsonInvalidIdentifierStart({ esbuild }) {
    ...
    const { code } = await esbuild.transform(`{ "\\uD835\\uDFCE": "y" }`, {
      loader: "json",
    });
  ...
  async text({ esbuild }) {
    const { code } = await esbuild.transform(`This is some text`, {
      loader: "text",
    });
  ...
  async textESM({ esbuild }) {
    ...
    assert.strictEqual(
      code,
      `var stdin_default = "This is some text";\nexport {\n  stdin_default as default\n};\n`
    );
  },

  async base64({ esbuild }) {
    const { code } = await esbuild.transform(`\x00\x01\x02`, {
      loader: "base64",
    });
  ...
  async dataurl({ esbuild }) {
    ...
    assert.strictEqual(
      code,
      `module.exports = "data:application/octet-stream;base64,AAEC";\n`
    );
  },

  async sourceMapTrueWithName({ esbuild }) {
    const { code, map } = await esbuild.transform(`let       x`, {
      sourcemap: true,
      sourcefile: "afile.js",
    });
  ...
  async sourceMapInlineWithName({ esbuild }) {
    ...
    await assertSourceMap(
      Buffer.from(base64.trim(), "base64").toString(),
      "afile.js"
    );
  },
  ...
  async numericLiteralPrinting({ esbuild }) {
    async function checkLiteral(text) {
      const { code } = await esbuild.transform(`return ${text}`, {
        minify: true,
      });
    ...
    for (let i = 0; i < 10; i++) {
      for (let j = 0; j < 10; j++) {
        promises.push(
          checkLiteral(`0.${"0".repeat(i)}${"123456789".slice(0, j)}`)
        );
        promises.push(
          checkLiteral(`1${"0".repeat(i)}.${"123456789".slice(0, j)}`)
        );
        promises.push(
          checkLiteral(`1${"123456789".slice(0, j)}${"0".repeat(i)}`)
        );
      }
    }
  ...
  async sourceMapControlCharacterEscapes({ esbuild }) {
    ...
    const { code, map } = await esbuild.transform(input, {
      sourcemap: true,
      sourcefile: "afile.code",
    });
  ...
  async transformLegalCommentsJS({ esbuild }) {
    assert.strictEqual(
      (await esbuild.transform(`//!x\ny()`, { legalComments: "none" })).code,
      `y();\n`
    );
    assert.strictEqual(
      (await esbuild.transform(`//!x\ny()`, { legalComments: "inline" })).code,
      `//!x\ny();\n`
    );
    assert.strictEqual(
      (await esbuild.transform(`//!x\ny()`, { legalComments: "eof" })).code,
      `y();\n//!x\n`
    );
    try {
  ...
  async pureCallPrint({ esbuild }) {
    const { code: code1 } = await esbuild.transform(`print(123, foo)`, {
      minifySyntax: true,
      pure: [],
    });
    assert.strictEqual(code1, `print(123, foo);\n`);

    const { code: code2 } = await esbuild.transform(`print(123, foo)`, {
      minifySyntax: true,
      pure: ["print"],
    });
  ...
  async pureCallConsoleLog({ esbuild }) {
    const { code: code1 } = await esbuild.transform(`console.log(123, foo)`, {
      minifySyntax: true,
      pure: [],
    });
    assert.strictEqual(code1, `console.log(123, foo);\n`);

    const { code: code2 } = await esbuild.transform(`console.log(123, foo)`, {
      minifySyntax: true,
      pure: ["console.log"],
    });
    assert.strictEqual(code2, `foo;\n`);
  ...
  async dynamicImportString({ esbuild }) {
    const { code } = await esbuild.transform(`import('foo')`, {
      target: "chrome63",
    });
  ...
  async dynamicImportStringES6({ esbuild }) {
    const fromPromiseResolve = (text) =>
      text.slice(text.indexOf("Promise.resolve"));
    const { code } = await esbuild.transform(`import('foo')`, {
      target: "chrome62",
    });
    assert.strictEqual(
      fromPromiseResolve(code),
  ...
  async dynamicImportStringES5Minify({ esbuild }) {
    ...
    assert.strictEqual(
      fromPromiseResolve(code),
      `Promise.resolve().then(function(){return __toESM(require("foo"))});\n`
    );
  },
  ...
  async dynamicImportExpressionES6({ esbuild }) {
    const fromPromiseResolve = (text) =>
      text.slice(text.indexOf("Promise.resolve"));
    const { code: code2 } = await esbuild.transform(`import(foo)`, {
      target: "chrome62",
    });
    assert.strictEqual(
      fromPromiseResolve(code2),
      `Promise.resolve().then(() => __toESM(require(foo)));\n`
    );
  },
  ...
  async dynamicImportExpressionES5({ esbuild }) {
    const fromPromiseResolve = (text) =>
      text.slice(text.indexOf("Promise.resolve"));
    const { code: code3 } = await esbuild.transform(`import(foo)`, {
      target: "chrome48",
    });
    assert.strictEqual(
      fromPromiseResolve(code3),
      `Promise.resolve().then(function() {\n  return __toESM(require(foo));\n});\n`
    );
  },
  ...
  async dynamicImportExpressionES5Minify({ esbuild }) {
    const fromPromiseResolve = (text) =>
      text.slice(text.indexOf("Promise.resolve"));
    const { code: code4 } = await esbuild.transform(`import(foo)`, {
      target: "chrome48",
      minifyWhitespace: true,
    });
    assert.strictEqual(
      fromPromiseResolve(code4),
      `Promise.resolve().then(function(){return __toESM(require(foo))});\n`
    );
  },
  ...
  async typeofEqualsUndefinedTarget({ esbuild }) {
    assert.strictEqual(
      (
        await esbuild.transform(`a = typeof b !== 'undefined'`, {
          minify: true,
        })
      ).code,
      `a=typeof b<"u";\n`
  ...
  async caseInsensitiveTarget({ esbuild }) {
    assert.strictEqual(
      (await esbuild.transform(`a ||= b`, { target: "eS5" })).code,
      `a || (a = b);\n`
    );
    assert.strictEqual(
      (await esbuild.transform(`a ||= b`, { target: "eSnExT" })).code,
      `a ||= b;\n`
    );
  },
  ...
  async multipleEngineTargets({ esbuild }) {
    const check = async (target, expected) =>
      assert.strictEqual(
        (await esbuild.transform(`foo(a ?? b)`, { target })).code,
        expected
      );
    await Promise.all([
  ...
  async multipleEngineTargetsNotSupported({ esbuild }) {
    try {
      await esbuild.transform(`0n`, {
        target: ["es5", "chrome1", "safari2", "firefox3"],
      });
      throw new Error("Expected an error to be thrown");
    } catch (e) {
      assert.strictEqual(
  ...
  async supported({ esbuild }) {
    const check = async (options, input, expected) => {
      try {
        assert.strictEqual(
          (await esbuild.transform(input, options)).code,
          expected
        );
      } catch (e) {
    ...
    await Promise.all([
      ...
      check(
        { supported: { arrow: true }, target: "es5" },
        `x = () => y`,
        `x = () => y;\n`
      ),
      check(
        { supported: { arrow: false }, target: "es5" },
        `x = () => y`,
        `x = function() {\n  return y;\n};\n`
      ),
      check(
        { supported: { arrow: true }, target: "es2022" },
        `x = () => y`,
        `x = () => y;\n`
      ),
      check(
        { supported: { arrow: false }, target: "es2022" },
        `x = () => y`,
        `x = function() {\n  return y;\n};\n`
      ),
      ...
      check(
        { supported: { bigint: true }, target: "es5" },
        `x = 1n`,
        `x = 1n;\n`
      ),
      check(
        { supported: { bigint: false }, target: "es5" },
        `x = 1n`,
        `Big integer literals are not available in the configured target environment ("es5" + 1 override)`
      ),
      check(
        { supported: { bigint: true }, target: "es2022" },
        `x = 1n`,
        `x = 1n;\n`
      ),
      check(
        { supported: { bigint: false }, target: "es2022" },
        `x = 1n`,
        `Big integer literals are not available in the configured target environment ("es2022" + 1 override)`
      ),

      // CSS: lower
      check(
        { supported: { "hex-rgba": true }, loader: "css" },
        `a { color: #1234 }`,
        `a {\n  color: #1234;\n}\n`
      ),
      check(
        { supported: { "hex-rgba": false }, loader: "css" },
        `a { color: #1234 }`,
      ...
      check(
        { supported: { bigint: false, arrow: true }, target: "es2022" },
        `x = 1n`,
        `Big integer literals are not available in the configured target environment ("es2022" + 2 overrides)`
      ),
    ]);
  },
  ...
  async regExpFeatures({ esbuild }) {
    const check = async (target, input, expected) =>
      assert.strictEqual(
        (await esbuild.transform(input, { target })).code,
        expected
      );
  ...
  forAwait: ({ esbuild }) =>
    futureSyntax(
      esbuild,
      "async function foo() { for await (let x of y) {} }",
      "es2017",
      "es2018"
    ),
  bigInt: ({ esbuild }) => futureSyntax(esbuild, "123n", "es2019", "es2020"),
  bigIntKey: ({ esbuild }) =>
    futureSyntax(esbuild, "({123n: 0})", "es2019", "es2020"),
  bigIntPattern: ({ esbuild }) =>
    futureSyntax(esbuild, "let {123n: x} = y", "es2019", "es2020"),
  nonIdArrayRest: ({ esbuild }) =>
    futureSyntax(esbuild, "let [...[x]] = y", "es2015", "es2016"),
  topLevelAwait: ({ esbuild }) =>
    futureSyntax(esbuild, "await foo", "es2020", "esnext"),
  topLevelForAwait: ({ esbuild }) =>
    futureSyntax(esbuild, "for await (foo of bar) ;", "es2020", "esnext"),

  // Future syntax: async generator functions
  asyncGenFnStmt: ({ esbuild }) =>
    futureSyntax(esbuild, "async function* foo() {}", "es2017", "es2018"),
  asyncGenFnExpr: ({ esbuild }) =>
    futureSyntax(esbuild, "(async function*() {})", "es2017", "es2018"),
  asyncGenObjFn: ({ esbuild }) =>
    futureSyntax(esbuild, "({ async* foo() {} })", "es2017", "es2018"),
  asyncGenClassStmtFn: ({ esbuild }) =>
    futureSyntax(esbuild, "class Foo { async* foo() {} }", "es2017", "es2018"),
  asyncGenClassExprFn: ({ esbuild }) =>
    futureSyntax(esbuild, "(class { async* foo() {} })", "es2017", "es2018"),
};
...
function registerClassPrivateTests(target) {
  ...
  transformTests[`tsTransformClassPrivate_${target[0]}`] = async ({
    esbuild,
  }) =>
    new Function(
      (await esbuild.transform(contents, { target, loader: "ts" })).code
    )();
...
let formatTests = {
  async formatMessages({ esbuild }) {
    ...
    assert.strictEqual(
      messages[0],
      `${errorIcon} [ERROR] This is an error\n\n`
    );
    assert.strictEqual(
      messages[1],
      `${errorIcon} [ERROR] Another error\n\n    file.js:0:0:\n      0 │ \n        ╵ ^\n\n`
    );
  },
};
...
`
  ...
  async transformSyncJSMapNoContent({ esbuild }) {
    const { code, map } = esbuild.transformSync(`1+2`, {
      sourcemap: true,
      sourcesContent: false,
    });
    assert.strictEqual(code, `1 + 2;\n`);
...
${path
  ...
  .replace(/\\/g, "/")}:1:2: ERROR: Unexpected end of file`
      );
...
`
    ...
    assert.strictEqual(
      esbuild.analyzeMetafileSync(metafile, { verbose: true }),
      `
  out.js ────── 100b ── 100.0%
   ├ lib.js ──── 50b ─── 50.0%
   │  └ entry.js
...
async function main() {
  ...
  let timeout = setTimeout(() => {
    console.error(
      `❌ js api tests timed out after ${minutes} minutes, exiting...`
    );
    process.exit(1);
  }, minutes * 60 * 1000);
  ...
  let allTestsPassed = (await Promise.all(tests.map(runTest))).every(
    (success) => success
  );
